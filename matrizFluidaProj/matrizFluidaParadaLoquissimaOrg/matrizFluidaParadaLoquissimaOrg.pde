float a,
s,  // distância de um ponto até a posição de cada célula da matriz. Usado atualmente no centro da tela, criando toda a concentricidade.
b,
c,
d,
t,
...
void draw(){
  ...
  for(float i = 0; i <= width + 0; i+= abs(map(((40*sin(d))), 0, 40, 20, 100))){
    ...
    for(float j =  (sin(k)*50); j <= height + (sin(k)*50); j += 7){
      ...
      t = map(s, 0, width/2, 0, mouseX); //por ser menor que a tela, cria loops na distancia. -- importante.
