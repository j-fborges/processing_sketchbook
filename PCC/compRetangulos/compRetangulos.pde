Grid g = new Grid();
float moduleXSz, moduleYSz, moduleSz, bTypeSz = 6, leadingSz = bTypeSz*1.25;
...
class Grid{
  ...
  for(int i = 0; i < gModuLnHeightN; i++){
    for(int j = 0; j < gModuLnWidthN; j++){
      dMsN +=1;
      gridMs[moduI] = new Module((j*leadingSz)+gXPos, (i*leadingSz)+gYPos, j, i, moduI);
      dMsI = (int[]) expand(dMsI, dMsN);
      moduI += 1;
...
void newRect(){
  rectsN += 1;
  rects = (RectObj[]) expand(rects, rectsN);
  rects[rectsN-1] = new RectObj();
  rects[rectsN-1].rI = rectsN-1;
  //rects[rectsN-1].rProp = random(.8,1.0);
  rects[rectsN-1].rProp = 1;
  //rPos = (int)random(dMsN);
  sortDRect();
  
  rects[rectsN-1].rMI = dMsN;
  g.gridMs[rPos].mRI = rectsN-1;
  
  rects[rectsN-1].rXPos = g.gridMs[rPos].mXPos;
  rects[rectsN-1].rYPos = g.gridMs[rPos].mYPos;
  
  rects[rectsN-1].rMXI = g.gridMs[rPos].mXI;
  rects[rectsN-1].rMYI = g.gridMs[rPos].mYI;
  
  rects[rectsN-1].maxMsLHorizontal = g.gModuLnWidthN - g.gridMs[rPos].mXI;
  rects[rectsN-1].maxMsLVertical = g.gModuLnHeightN - g.gridMs[rPos].mYI;
  
  for(int i = 0; i< rects[rectsN-1].maxMsLHorizontal; i++){
    
    if(g.gridMs[rPos + i].occupied == false){
      if(rects[rectsN-1].foundOccupiedX == false){
        rects[rectsN-1].totalDMsX +=1;
      }
    } else {
     rects[rectsN-1].foundOccupiedX = true;
    }
  }
  rects[rectsN-1].dMsX = new int[rects[rectsN-1].totalDMsX];
  
  for(int i = 0; i< rects[rectsN-1].maxMsLVertical; i++){
    if(g.gridMs[rPos + i*g.gModuLnWidthN].occupied == false){
      if(rects[rectsN-1].foundOccupiedY == false){
        rects[rectsN-1].totalDMsY +=1;
      }
    } else {
      rects[rectsN-1].foundOccupiedY = true;
    }
  }
  rects[rectsN-1].dMsY = new int[rects[rectsN-1].totalDMsY];
  
  if(rects[rectsN-1].maxMsLHorizontal <= rects[rectsN-1].maxMsLVertical){
    rects[rectsN-1].xAxisBigger = false;
  } else { 
    rects[rectsN-1].xAxisBigger = true;
  }
  
  if(rects[rectsN-1].xAxisBigger == true){
    rects[rectsN-1].adjDMsXFoundOccupied = new boolean[rects[rectsN-1].totalDMsX];
    rects[rectsN-1].adjacentMs = new int[rects[rectsN-1].totalDMsX];
    for(int i = 0; i< rects[rectsN-1].totalDMsX-1; i++){
      for(int j = 0; j < rects[rectsN-1].totalDMsY-1; j++){
        test = rPos+i+(j*g.gModuLnWidthN);
        if(g.gridMs[rPos+i+(j*g.gModuLnWidthN)].occupied == false){
  ...
  if(rects[rectsN-1].xAxisBigger == false){
    ...
    for(int i = 0; i< rects[rectsN-1].totalDMsY-1; i++){
      for(int j = 0; j < rects[rectsN-1].totalDMsX-1; j++){
        if(g.gridMs[rPos+j+(i*g.gModuLnWidthN)].occupied == false){
  ...
  rects[rectsN-1].rPI = (int) random(rects[rectsN-1].possibilitiesN);
