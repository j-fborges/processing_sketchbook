...
class Sqr{
 ...
 void init(){
  ...
  sDirAngle = spots[rSpot].sDirAngle;
  
  //if(refA > -(PI/4) && refA < (PI/4)
  
  
  int xDir = 1, yDir = 0;
  if(sDirAngle == 0 || sDirAngle == TWO_PI){
    xDir = +1;
    yDir = 0;
  } else 
  if(sDirAngle == HALF_PI){
    xDir = 0;
    yDir = +1;
  } else
  if(sDirAngle == HALF_PI*3){
    xDir = 0;
    yDir = -1;
  } else
  if(sDirAngle == PI){
...
class Spot{
 ...
 void firstInit(int i){
   ...
   sDirAngle = sqrs[sqrsN-1].anchorsAngles[i];
 ...
 void init(int i){
   ...
   sDirAngle = sqrs[sqrsN-1].anchorsAngles[i-pSpotsN];
