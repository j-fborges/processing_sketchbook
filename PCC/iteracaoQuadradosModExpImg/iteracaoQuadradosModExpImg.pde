...
float uniSz = 4,// "universal size" ------------ tamanho do modulo geral, que cria o grid.
uniMarginSz = 0,       //  "universal margin size" ------------ tamanho da margem geral de todos os objetos
uniASzMax =5,         // "universal acutal size maximum" ------------ Tamanho maximo do objeto que vai ser desenhado dentro ou fora do espaço do modulo. Os tamanhos são gerados a partir de randomização, tendo um máximo ou mínimo
uniASzMin =2;         // "universal actual size minimum" ------------- Tamanho minimo.
float cDSz = 4;              // "colision detection size" ---------------- Distancia para detecção de disponibilidade para cada "spot".
float uniXSz = uniSz, uniYSz = (uniSz/prop);


int sqrsN, spotsN, pSpotsN = 1, occupiedN;
int tR = 5;
String cat1 = "ilha", cat2 = "AA";
...
void setup(){
 ...
 background(255);
 frameRate(60*3);
 img = loadImage("ilhaSilhu-01.jpg");
 rectMode(RADIUS);
 result = createGraphics(width, height, SVG, "data/"+ cat1 +"/"+ cat2 +"/"+ nf(millis())+ nf(second()) + nf(hour())+ nf(month())+cat1+ cat2 +"result.svg" );
 //beginRecord(SVG, nf(millis())+ nf(second()) + nf(hour())+ nf(month()) +"result.svg");
 result.beginDraw();
 result.rectMode(RADIUS);
...
void mousePressed(){
 ...
 result.dispose();
 //result.endDraw();
 //result.save(nf(millis())+ nf(second()) + nf(hour())+ nf(month()) +"result.png");
...
class Sqr{
 int rSpot, sXDir, sYDir, rDSpotI;
 PVector[] anchors;
 PVector loc;
 int[] anchorXDir, anchorYDir, anchorXDirI, anchorYDirI;
 int xDirValI= 0, yDirValI = 0;
 float refXPos, refYPos, xPos, yPos, sz, xSz, ySz, marginSz, aSz, aXSz, aYSz;
 ...
 void firstInit(){
  ...
  aSz = random(uniASzMin,uniASzMax);
  aXSz = aSz;
  aYSz = aXSz/prop;
  sz = uniSz;
  xSz = sz;
  ...
  for(int i = 0; i<anchors.length; i++){
   ...
   anchors[i].x = xPos + (anchorXDir[i])*(xSz+marginSz);
   anchors[i].y = yPos + (anchorYDir[i])*(ySz+marginSz);
  }
 }
 ...
 void disp(){
   noStroke();
   //fill(#28283C);
   fill(img.get((int)xPos, (int)yPos));
   rect(xPos, yPos, aXSz,aYSz);
   
   result.noStroke();
   //result.fill(#28283C);
   result.fill(img.get((int)xPos, (int)yPos));
   result.rect(xPos, yPos, aXSz,aYSz);
 }
}

class Spot{
 int  sSqrIndex, sXDir, sYDir , sXDirI, sYDirI;
 boolean occupied;
 PVector loc;
 ...
 void firstInit(int i){
   ...
   xPos = sqrs[sqrsN-1].anchors[i].x; //valor do ponto de referencia já somado com o raio do quadrado e a largura da margem.
   yPos = sqrs[sqrsN-1].anchors[i].y;
 ...
 void init(int i){
   ...
   xPos = sqrs[sqrsN-1].anchors[i-pSpotsN].x;
   yPos = sqrs[sqrsN-1].anchors[i-pSpotsN].y;
...
void check(){
  ...
  for(int j = 0; j < dSpotI.length; j+=1){
    for(int i = 0; i < sqrsN; i+=1){
      ...
      float spotXC = spots[dSpotI[j]].xPos + (spots[dSpotI[j]].sXDir*(uniXSz +uniMarginSz));
      float spotYC = spots[dSpotI[j]].yPos + (spots[dSpotI[j]].sYDir*(uniYSz +uniMarginSz));
      //println("occupied");
