...
void keyPressed(){
   ...
   switch(keyCode){
        ...
        println(fontList[chars[charIndex].fontIndex]);
        ...
        switch(key){
          ...
          case 'o':
          for(int i = 0; i < typedString.length(); i ++){
          chars[i].ra1 = radians(random(-7, 7));
          chars[i].ra2 = radians(random(-7, 7));
          }
...
void keyReleased(){
  ...
  switch(key){
    case 'z':
          ...
          case 'x':
          int rf = (int)random(fontList.length);
          for(int i = 0; i <typedString.length(); i++){
          chars[i].fontIndex = rf;
          }
          break;
  }
  switch(keyCode){
