...
float p, h;
int vn = 9, mn = 30, mss = 10;
PVector vs[] = new PVector[vn], mp [] = new PVector[vn], fmp [] = new PVector[vn];
float as[] = new float[vn], s[] = new float[vn] , sg [] = new float[vn], sc [] = new float[vn] , ss [] = new float[vn];
float r = 300;
float vsx[] =  new float[vn], vsy[] =  new float [vn];
void setup(){
  size(1000,1000, P2D);
  ...
  for(int i = 0; i < vn; i++){
   ...
   vsx[i] = random(-1,1);
   vsy[i] = random(-1,1);
   vs[i] = new PVector(vsx[i]*r, vsy[i]*r); 
  }
...
void draw(){
  ...
  for(int i = 0; i < vn; i++){
    ...
    vsx[i] += random(-.005,.005);
    vsy[i] += random(-.005,.005);
  ...
  for(int i = 0; i < vn; i++){
    as[i]= atan2(mp[i].y, mp[i].x);
   fmp[i].x = mp[i].x + (cos(as[i])*s[i]*ss[i]);
   fmp[i].y = mp[i].y + (sin(as[i])*s[i]*ss[i]);
  }
